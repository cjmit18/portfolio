...
const PHASES = [
  { id:"p0", title:"Environment & foundation", weeks:"Weeks 1-2", color:G.mid,
    items:[
  ...
  { id:"p1", title:"Data models & repositories", weeks:"Weeks 3-5", color:B.mid,
    items:[
  ...
  { id:"p2", title:"Business logic services", weeks:"Weeks 6-8", color:P.mid,
    items:[
  ...
  { id:"p3", title:"Qt UI — core views", weeks:"Weeks 9-12", color:A.mid,
    items:[
  ...
  { id:"p4", title:"Scheduling & assignment UI", weeks:"Weeks 13-15", color:T.mid,
    items:[
  ...
  { id:"p6", title:"Polish & deployment", weeks:"Weeks 19-21", color:R.mid,
    items:[
...
const ROADMAP = [
  { version:"v1.0", label:"MVP", color:G,
  ...
  { version:"v1.5", label:"Production hardening", color:B,
  ...
  { version:"v2.0", label:"Compliance & integration", color:P,
  ...
  { version:"v3.0+", label:"Long-term vision", color:A,
...
function tag(c, label) {
  return <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:c.light,color:c.dark,fontFamily:"var(--font-mono)"}}>{label}</span>;
}

function Card({children, accent, style={}}) {
  return <div style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderLeft:accent?`3px solid ${accent}`:undefined,borderRadius:"var(--border-radius-lg)",padding:"18px 20px",marginBottom:14,...style}}>{children}</div>;
}

function CardH({children}) { return <div style={{fontSize:14,fontWeight:500,color:"var(--color-text-primary)",marginBottom:4}}>{children}</div>; }
function CardS({children}) { return <div style={{fontSize:12,color:"var(--color-text-tertiary)",fontFamily:"var(--font-mono)",marginBottom:14}}>{children}</div>; }
...
function G3({children,gap=12}) {
  return <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap}}>{children}</div>;
}

function Sec({title,sub}) {
  return <div style={{marginBottom:24}}>
    <h2 style={{fontSize:20,fontWeight:500,color:"var(--color-text-primary)",margin:"0 0 5px"}}>{title}</h2>
    {sub&&<p style={{fontSize:14,color:"var(--color-text-secondary)",margin:0,lineHeight:1.6}}>{sub}</p>}
  </div>;
}

function OverviewTab() {
  return <div>
    <Sec title="Project overview" sub="A purpose-built operations tool for regulated cannabis kitchen environments — well beyond generic staff tracking." />
    <G2>
      <Card>
        <CardH>The problem</CardH><CardS>// what this app solves</CardS>
      ...
      <Card>
        <CardH>Industry context</CardH><CardS>// cannabis kitchen specifics</CardS>
    ...
    <Card>
      <CardH>Suggested app names</CardH><CardS>// pick one or bring your own</CardS>
      <G3 gap={10}>
        {[["StationIQ","Intelligent station assignment. Clean and professional."],["FloorOps","Ops-focused, kitchen-floor feel. Simple and memorable."],["CertTrack","Puts training and certification front and center."],["KitchenGrid","References the assignment grid that is the core visual."],["PostUp","Kitchen slang for take your station. Unexpected and memorable."],["DispatchPro","Emphasizes the daily dispatch and assignment workflow."]].map(([n,d])=>(
          <div key={n} style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"12px 14px"}}>
            <div style={{fontSize:14,fontWeight:500,color:G.text,marginBottom:4}}>{n}</div>
...
function StackTab() {
  return <div>
    <Sec title="Tech stack" sub="Primary recommendation: Qt6 desktop app. All dependencies are open source, cross-platform, and battle-tested." />
    <Card>
      <CardH>Primary stack — Qt6 desktop</CardH>
      <CardS>// recommended for a single-location kitchen workstation or manager laptop</CardS>
      <G2 gap={12}>
        {STACK.map(s=>(
          <div key={s.name} style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"14px 16px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
              <span style={{fontSize:14,fontWeight:500,color:"var(--color-text-primary)"}}>{s.name}</span>
              {tag(s.tagC,s.tag)}
            </div>
            <div style={{fontSize:11,color:G.text,fontFamily:"var(--font-mono)",marginBottom:8}}>{s.role}</div>
    ...
    <Card>
      <CardH>Alternative stack — web-based (Drogon)</CardH>
      <CardS>// choose this path if you need tablet access across the kitchen or multi-user from day one</CardS>
      <G2 gap={12}>
        {[["Drogon","C++ web backend",B,"High-performance C++ HTTP/WebSocket framework. Choose this path if you want the app accessible from any tablet or computer on the kitchen network."],["PostgreSQL","Server database",B,"Pairs with Drogon for multi-user network-accessible storage. More to manage than SQLite but valuable for multi-location or concurrent access."],["libpqxx","PostgreSQL C++ client",T,"Official PostgreSQL C++ client. Replaces Qt SQL in the web-backend path. RAII-based API with full async support."],["React / HTML","Web frontend",T,"A thin web UI served by the Drogon backend. Separates display from core C++ logic and works from any browser including kitchen tablets."]].map(([n,r,c,d])=>(
          <div key={n} style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"14px 16px",opacity:0.85}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
              <span style={{fontSize:14,fontWeight:500,color:"var(--color-text-primary)"}}>{n}</span>
              {tag(c,"optional")}
            </div>
            <div style={{fontSize:11,color:B.text,fontFamily:"var(--font-mono)",marginBottom:8}}>{r}</div>
            <div style={{fontSize:12,color:"var(--color-text-secondary)",lineHeight:1.6}}>{d}</div>
          </div>
        ))}
      </G2>
    </Card>
    <G2>
      ...
      <Card>
        <CardH>CMakeLists skeleton</CardH>
        <CardS>// starting point for your root CMakeLists.txt</CardS>
        <div style={{background:"var(--color-background-tertiary)",borderRadius:"var(--border-radius-md)",padding:14,fontFamily:"var(--font-mono)",fontSize:11,color:"var(--color-text-secondary)",lineHeight:1.9}}>
          <span style={{color:"var(--color-text-tertiary)"}}>cmake_minimum_required(VERSION 3.25)</span><br/>
          <span style={{color:"var(--color-text-tertiary)"}}>project(StationIQ VERSION 0.1)</span><br/>
          <span style={{color:"var(--color-text-tertiary)"}}>set(CMAKE_CXX_STANDARD 20)</span><br/><br/>
          <span style={{color:G.text}}>find_package</span>(Qt6 REQUIRED COMPONENTS<br/>
          {"  "}Core Widgets Sql Charts PrintSupport)<br/><br/>
          <span style={{color:"var(--color-text-tertiary)"}}># nlohmann/json and Google Test</span><br/>
          <span style={{color:G.text}}>include</span>(FetchContent)<br/>
          FetchContent_Declare(json URL ...)<br/>
          FetchContent_Declare(googletest URL ...)<br/>
          FetchContent_MakeAvailable(json googletest)<br/><br/>
          <span style={{color:G.text}}>add_subdirectory</span>(src)<br/>
          <span style={{color:G.text}}>add_subdirectory</span>(tests)
        </div>
      </Card>
    </G2>
  </div>;
...
function ArchTab() {
  return <div>
    ...
    <Card>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4,maxWidth:520,margin:"0 auto"}}>
        {[[G,"Qt UI layer","MainWindow · views · dialogs · models · charts"],[P,"Service layer","EmployeeService · TrainingService · AssignmentEngine · SuggestionEngine"],[B,"Repository layer","EmployeeRepo · StationRepo · TrainingRepo · ScheduleRepo · AssignmentRepo"],[A,"DatabaseManager","SQLite 3 via Qt SQL · connection singleton · schema migrations"]].map(([c,l,d],i)=>(
          <div key={l}>
            <div style={{textAlign:"center",background:c.light,border:`0.5px solid ${c.mid}`,borderRadius:"var(--border-radius-md)",padding:"12px 16px"}}>
              <div style={{fontSize:14,fontWeight:500,color:c.dark}}>{l}</div>
              <div style={{fontSize:11,color:c.text,fontFamily:"var(--font-mono)",marginTop:4}}>{d}</div>
            </div>
            {i<3&&<div style={{textAlign:"center",color:"var(--color-text-tertiary)",fontSize:16,lineHeight:1.2}}>↕</div>}
          </div>
        ))}
      </div>
    </Card>
    <Card>
      <CardH>Core modules</CardH>
      <CardS>// nine modules, each with a single responsibility</CardS>
      <G3 gap={10}>
        {MODULES.map(([n,d])=>(
          <div key={n} style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"11px 13px"}}>
            <div style={{fontSize:12,fontWeight:500,color:G.text,fontFamily:"var(--font-mono)",marginBottom:5}}>{n}</div>
            <div style={{fontSize:11,color:"var(--color-text-secondary)",lineHeight:1.5}}>{d}</div>
          </div>
        ))}
      </G3>
    </Card>
    <G2>
      <Card>
        <CardH>Assignment algorithm</CardH>
        <CardS>// greedy constraint satisfaction, O(S x E)</CardS>
        <div style={{background:"var(--color-background-tertiary)",borderRadius:"var(--border-radius-md)",padding:14,fontFamily:"var(--font-mono)",fontSize:11,color:"var(--color-text-secondary)",lineHeight:1.9}}>
          <span style={{color:P.text}}>assign</span>(schedule, employees, records):<br/><br/>
          <span style={{color:"var(--color-text-tertiary)"}}>  // critical stations processed first</span><br/>
          {"  "}<span style={{color:G.text}}>for</span> station in schedule.sorted(priority):<br/>
          {"    "}eligible = records.filter(<br/>
          {"      "}station == station<br/>
          {"      "}AND level &gt;= Certified AND NOT expired)<br/><br/>
          {"    "}trainers = eligible.filter(level == Trainer)<br/>
          {"    "}certified = eligible.filter(level == Certified)<br/><br/>
          {"    "}assign(trainers.take(required))<br/>
          {"    "}assign(certified.fill_remaining)<br/>
          {"    "}<span style={{color:G.text}}>if</span> trainer_assigned:<br/>
          {"      "}assign(inTraining.take(1))<br/><br/>
          {"    "}<span style={{color:G.text}}>if</span> understaffed: flag(WARNING)<br/><br/>
          <span style={{color:"var(--color-text-tertiary)"}}>  // pass surplus to SuggestionEngine</span><br/>
          {"  "}<span style={{color:P.text}}>suggest</span>(employees - assigned, deficits)
        </div>
      </Card>
      <Card>
        <CardH>Suggestion algorithm</CardH>
        <CardS>// rank cross-training opportunities for surplus staff</CardS>
        <div style={{background:"var(--color-background-tertiary)",borderRadius:"var(--border-radius-md)",padding:14,fontFamily:"var(--font-mono)",fontSize:11,color:"var(--color-text-secondary)",lineHeight:1.9}}>
          <span style={{color:P.text}}>suggest</span>(surplus, deficits):<br/><br/>
          {"  "}<span style={{color:G.text}}>for</span> employee in surplus:<br/>
          {"    "}candidates = []<br/><br/>
          {"    "}<span style={{color:G.text}}>for</span> station in allStations:<br/>
          {"      "}level = employee.levelAt(station)<br/>
          {"      "}<span style={{color:G.text}}>if</span> level &lt; Certified:<br/><br/>
          <span style={{color:"var(--color-text-tertiary)"}}>        // urgency + proximity score</span><br/>
          {"        "}score = deficit[station] * <span style={{color:A.text}}>0.7</span><br/>
          {"               "}+ proximity[level] * <span style={{color:A.text}}>0.3</span><br/>
          {"        "}<span style={{color:G.text}}>if</span> trainer_here_today:<br/>
          {"          "}score += <span style={{color:A.text}}>0.2</span><br/><br/>
          {"        "}candidates.append(station, score)<br/><br/>
          {"    "}yield employee, candidates.top(<span style={{color:A.text}}>3</span>)
        </div>
      </Card>
    </G2>
    <Card>
      <CardH>Key database entities</CardH>
      <CardS>// schema concepts — full SQL lives in docs/schema.sql</CardS>
      <G3 gap={10}>
        {[["employees",["id","name","employee_number","hire_date","status","notes"]],["stations",["id","name","category","min_staff","max_staff","requires_supervisor"]],["training_records",["id","employee_id","station_id","level","trained_date","trained_by","expiry_date"]],["production_schedules",["id","date","status (Draft/Active/Completed)"]],["schedule_stations",["schedule_id","station_id","batch_count","required_staff","priority"]],["daily_assignments",["id","schedule_id","employee_id","station_id","method","created_at"]]].map(([t,f])=>(
          <div key={t} style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"11px 13px"}}>
            <div style={{fontSize:12,fontWeight:500,color:B.text,fontFamily:"var(--font-mono)",marginBottom:6}}>{t}</div>
            {f.map(field=><div key={field} style={{fontSize:11,color:"var(--color-text-tertiary)",fontFamily:"var(--font-mono)",lineHeight:1.8}}>{field}</div>)}
          </div>
        ))}
      </G3>
    </Card>
...
function RoadmapTab() {
  return <div>
    <Sec title="Feature roadmap" sub="Start with v1.0 and build outward. Every item here solves a real problem in a regulated kitchen environment." />
    {ROADMAP.map(r=>(
      <Card key={r.version} accent={r.color.mid}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
          <span style={{fontSize:16,fontWeight:500,color:"var(--color-text-primary)"}}>{r.version}</span>
          {tag(r.color,r.label)}
        </div>
        <p style={{fontSize:13,color:"var(--color-text-secondary)",marginBottom:14,lineHeight:1.6}}>{r.desc}</p>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {r.features.map((f,i)=>(
            <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start"}}>
              <span style={{color:r.color.mid,fontFamily:"var(--font-mono)",fontSize:11,marginTop:2,flexShrink:0}}>-&gt;</span>
              <span style={{fontSize:12,color:"var(--color-text-secondary)",lineHeight:1.6}}>{f}</span>
            </div>
          ))}
        </div>
      </Card>
...
function ChecklistTab({completed,toggle,expanded,togglePhase}) {
  ...
  return <div>
    <Sec title="Development checklist" sub="7 phases, 66 tasks. Work through them in order — each phase depends on the previous one being solid. Click any item to mark it done." />
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:12,marginBottom:24}}>
      {[[pct+"%","overall progress",true],[done,"tasks completed",false],[totalItems-done,"tasks remaining",false],[7,"phases",false]].map(([n,l,bar])=>(
        <div key={l} style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"14px 16px"}}>
          <div style={{fontSize:26,fontWeight:500,color:G.text}}>{n}</div>
          <div style={{fontSize:11,color:"var(--color-text-tertiary)",fontFamily:"var(--font-mono)",marginTop:4}}>{l}</div>
          {bar&&<div style={{marginTop:8,height:3,background:"var(--color-border-tertiary)",borderRadius:2,overflow:"hidden"}}>
            <div style={{height:"100%",background:G.mid,width:pct+"%",borderRadius:2,transition:"width .3s"}}/>
          </div>}
        </div>
      ))}
    </div>
    {PHASES.map(phase=>{
      ...
      return <div key={phase.id} style={{marginBottom:12}}>
        <div onClick={()=>togglePhase(phase.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:isOpen?"8px 8px 0 0":8,cursor:"pointer",userSelect:"none"}}>
          <div style={{width:8,height:8,borderRadius:"50%",background:pDone===phase.items.length?G.mid:phase.color,flexShrink:0}}/>
          <span style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)",flex:1}}>{phase.title}</span>
          <div style={{width:60,height:3,background:"var(--color-border-tertiary)",borderRadius:2,overflow:"hidden"}}>
            <div style={{height:"100%",background:phase.color,width:Math.round(pDone/phase.items.length*100)+"%",borderRadius:2,transition:"width .3s"}}/>
          </div>
          <span style={{fontSize:11,fontFamily:"var(--font-mono)",color:"var(--color-text-secondary)",background:"var(--color-background-secondary)",padding:"2px 8px",borderRadius:4}}>{pDone}/{phase.items.length}</span>
          <span style={{fontSize:11,color:"var(--color-text-tertiary)",fontFamily:"var(--font-mono)"}}>{phase.weeks}</span>
          <span style={{color:"var(--color-text-tertiary)",fontSize:10,display:"inline-block",transform:isOpen?"rotate(180deg)":"none",transition:"transform .2s"}}>▼</span>
        </div>
        {isOpen&&<div style={{border:"0.5px solid var(--color-border-tertiary)",borderTop:"none",borderRadius:"0 0 8px 8px",overflow:"hidden"}}>
          {phase.items.map((item,idx)=>{
            const isDone = completed.has(item.id);
            const pc = priC[item.prio];
            return <div key={item.id} onClick={()=>toggle(item.id)} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"9px 14px",borderBottom:idx<phase.items.length-1?"0.5px solid var(--color-border-tertiary)":"none",background:isDone?"var(--color-background-secondary)":"var(--color-background-primary)",cursor:"pointer",opacity:isDone?0.5:1,userSelect:"none"}}>
              <div style={{width:14,height:14,borderRadius:3,border:isDone?"none":"0.5px solid var(--color-border-secondary)",background:isDone?G.mid:"transparent",flexShrink:0,marginTop:2,display:"flex",alignItems:"center",justifyContent:"center"}}>
                {isDone&&<span style={{color:"white",fontSize:9,fontWeight:500}}>✓</span>}
              </div>
              <span style={{fontSize:12,color:"var(--color-text-secondary)",lineHeight:1.6,flex:1,textDecoration:isDone?"line-through":"none"}}>{item.text}</span>
              <span style={{fontSize:9,padding:"2px 6px",borderRadius:3,background:pc.bg,color:pc.text,fontFamily:"var(--font-mono)",flexShrink:0,marginTop:3}}>{priL[item.prio]}</span>
            </div>;
          })}
        </div>}
      </div>;
